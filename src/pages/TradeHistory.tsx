  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">Todas as operações realizadas</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="bg-card border-border">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-2xl font-bold font-display">{recentTrades.length}</p>
          </CardContent>
        </Card>
        <Card className="bg-card border-border">
          <CardContent className="p-4 text-center">
        ...
        <Card className="bg-card border-border">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-2xl font-bold font-mono text-profit">
              {((recentTrades.filter(t => t.pnl && t.pnl > 0).length / recentTrades.filter(t => t.pnl !== undefined).length) * 100).toFixed(1)}%
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Select value={typeFilter} onValueChange={setTypeFilter}>
          <SelectTrigger className="w-[140px] bg-card border-border"><Filter className="h-4 w-4 mr-2" /><SelectValue /></SelectTrigger>
          <SelectContent>
              ...
              <tbody>
                {filtered.map((trade) => (
                  <tr key={trade.id} className="border-b border-border/50 hover:bg-accent/30 transition-colors">
                    <td className="p-4 text-muted-foreground text-xs">{trade.timestamp.toLocaleString('pt-BR')}</td>
                    <td className="p-4 font-medium font-mono">{trade.pair}</td>
                    <td className="p-4">
                      <Badge variant="outline" className={`text-xs ${trade.type === 'buy' ? 'border-profit text-profit' : 'border-loss text-loss'}`}>
                        {trade.type === 'buy' ? 'Compra' : 'Venda'}
                      </Badge>
                    </td>
                    <td className="p-4 text-right font-mono">${trade.price.toLocaleString()}</td>
                    <td className="p-4 text-right font-mono">{trade.amount}</td>
                    <td className="p-4 text-right font-mono">${trade.total.toLocaleString()}</td>
                    <td className={`p-4 text-right font-mono ${trade.pnl && trade.pnl >= 0 ? 'text-profit' : 'text-loss'}`}>
                      {trade.pnl ? `${trade.pnl >= 0 ? '+' : ''}$${trade.pnl.toFixed(2)}` : '—'}
                    </td>
                    <td className="p-4 text-right">
                      <Badge variant="secondary" className="text-xs">
                        {trade.status === 'completed' ? 'Completo' : trade.status === 'pending' ? 'Pendente' : 'Cancelado'}
                      </Badge>
                    </td>
                  </tr>
                ))}
